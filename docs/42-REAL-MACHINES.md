...
每一条都会改变某个物种的身份或这个项目的一条规矩，按 `AGENTS.md` 的改动纪律，它们不是工程决定。

1. **`porcelain` 从 Optimus 解绑，归入「虚」。**（§2）
   代价：`docs/31 §1` 第 3 行要改，「劳工人」那一档变成空的。
   不解绑的代价写在 §2 末尾。
2. **`patrol`：重取 Spot，还是把记录改成 ANYmal。**（§5 末）
   推荐 (a)。这一条不做，作品里最有辨识度的那个物种会一直名实不符。
3. **命名：接受 §5 的建议（保名 + `machine` 字段 + 档案页第二行），还是改名。**
   这是项目负责人那句「把真实的机器人命名好」的落点，**只有他能裁**。
   `id` 不动这一条不在裁定范围内，它是 seed 码的硬约束。
4. **`docs/26 §H` 是否加第三档「实·无几何」。**（§0 第一条）
   不加，「想象」会继续当垃圾桶用，而 §H 的力量来自它不是垃圾桶。
5. **Atlas 用 DRC 时代的 v5 几何，可不可以。**（§3）
   授权干净（BSD-3，LICENSE 与网格同目录），但版权人是 MIT CSAIL 而非 Boston Dynamics，
   而且那是液压的那一代。可以接受，但要写进 `ATTRIBUTION.md`，不要含糊。
6. **`wheelleg` 用 LimX 的轮足四足代 W1，可不可以。**（§3）
   和 Cassie 代 Digit 同类，同厂、同拓扑、Apache-2.0。
7. **`droid` 是否接受社区复刻件。** 本页建议**不接受**，理由在 §3 末。
   若维护者不同意，那要先改 §H 的措辞 —— 现在那句话不支持复刻品。
