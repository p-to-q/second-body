...
> **实时交互装置**：摄像头 → 人体姿态 → 一具等身的合成身体长在你的骨架上。
> 对标并逆向 Universal Everything《Future You》(Barbican, 2019)，
> 但把 2019 年不存在的那一层——**实时 AI 3D 生成**——真正放进交互回路。
>
> 线上：<https://useeme.ptoq.io>（只有快回路）。仓库、根包与 workspace scope
> 都叫 `see-me-see-u` / `@smu/*`。
>
> **这份写的是「为什么做」和「做成什么样才算成」，不是状态。**
> 哪些真的跑通了，只看 [`10-SURFACES.md`](10-SURFACES.md)；
> 「做完」的定义在 [`PRD.md`](PRD.md)；全部文档的地图在 [`index.md`](index.md)。
> 三十秒读完这一份，就可以开工（`../AGENTS.md` 的读取路线第 1 条）。
...
- **不做** 逐帧/实时 3D 生成（技术上不存在，别浪费时间）
- **不做** 网格蒙皮 / 自动绑骨（刚体挂载已足够，UniRig 只作为 P3 备选）
- ~~**不做** 多人同时交互（v1 单人。多人检测到了就选面积最大的那个）~~
  → **以单人为主，兼容多人**（作品负责人 2026-09-14）：默认仍是一个观众、选面积最大的那个；
  `?people=2|3` 时每人一具同伴身体，主观众身份稳定。见 `docs/50-MULTI-PERSON.md`、`PRD §4.3`
- **不做** 手部/面部细节追踪（身体尺度的作品，手指没人看得见）
- **不做** 训练任何模型
