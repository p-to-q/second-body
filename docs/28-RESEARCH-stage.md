...
- **身体是"陷"进地里，不是"浮"在空中 —— 而且这一条我一开始判错了。**
  我从 `scene-gallery.png` 上目测"脚部件比骨架落点高约 14cm"，写进了这份文档。
  装配那条线在 `/dev/figure.html?theme=porcelain&seed=1&tier=2&debug=1` 上量了真数：

  ```
  shin 世界包围盒  y ∈ [0.089, 0.511]     踝到膝，正确
  foot 世界包围盒  y ∈ [-0.025, 0.170]    最低点在地面**以下** 2.5cm
  骨架            footIdx.y = 0.030
  ```

  我看到的 0.17 是脚的**顶**，底下那截沉在暗地面里没看出来。**没有 14cm 的缝，
  有的是 2.5cm 的陷**；真实运行时 `core/stabilize.ts` 的 `ground()` 把 `footIdx`
  归零之后还会再沉 ~3cm，合计脚底约在 `y = −0.055`。
  根因是"贴地"贴的是**骨架点**，而观众看到的是**网格**。

  **这个修法不归舞台。** 它需要装配时每件的 `aabb` 才能算出脚的世界最低点
  （`src/creature/assemble.ts`，本轮明令不碰），而在舞台层猜一个偏移量
  = 把影子画在没有脚的地方。舞台这边做的是**把参照系换对**：
  接触阴影的"地面"从"身体自己的最低点"改成 **`y = 0`（地面本身）**。
  于是陷进去的脚照常有接触阴影（`lift ≤ 0`），真的跳起来时接触阴影**正确地消失**
  —— 而第一版那种自归一化的写法会强迫每具身体都有接触阴影，哪怕它在半空。

  > **给装配那条线的接口约定**：如果你们给整具身体加一个 Y 偏移，
  > **同一个偏移必须也加在喂给 `stage.frame(skeleton)` 的那副骨架上**。
  > 舞台的接触阴影读的是骨架端点的世界坐标，网格挪了骨架没挪，
  > 影子就会留在原地。两者同源是这条链能对上的唯一前提。
