...
| 亮底（`Walking City`）和暗底（`Future You` 展场照）**两种都成立**，但**从不混合** | 五套里 `paper` / `gallery` 是亮底、其余三套是暗底；**没有一套是中灰**。改前那张恰恰是中灰 |
...
五套场景取 0.092 / 0.078 / 0.105 / 0.120 / 0.130，上限 `STAGE.fogDensityMax = 0.16` ——
...
| 场景 | 天幕 | 布光 | 服务于 | 情绪 |
|---|---|---|---|---|
| `paper` 纸 | 亮（`skyGlow` 0.956 线性 = `#fafafa`）| 顶光 + 大补光，`rim` 压到 0.35，主光 ~80° | 任何物种 | 被印在纸上的一个形；没有房间、没有气氛、没有时间 |
...
## 为什么亮底有两套：`paper` 和 `gallery`

它们只差一个字段，`tint`。而那个字段是全部。

`gallery` 的天幕跟着物种的主光染色（`tint: 0.25`）—— 那是摄影棚的道理：
无缝背景纸会把打在它上面的光反回来，一具暖色的身体确实会把整间屋子染成米色。
它是**一个房间**，房间会被里面的东西影响。

`paper` 的 `tint` 是 **0**。不管站上去的是谁，底永远是 `#fafafa` ——
和展签、选择页、logo 用的是同一个值。它不是房间，是**纸**。
于是整件作品从头到尾只有一种白，而身体是那张纸上唯一的墨。

这条区别不是配色偏好，是两种世界观：
房间里有气氛，纸上没有。一件关于"一个我可以栖居于多少种身体"的作品
需要一个**不表态**的底 —— 纸就是那个不表态。

代价，写在这里免得下一个人重新发现一遍：

| 亮底上失效的东西 | 处理 |
|---|---|
| 加性混合的粒子 | `particle: 0`。明说这套没有，而不是给一个看不见的值 |
| 辉光 | `bloom: 0.12`（全场最低）。亮底上的 bloom 只会把剪影的边吃掉，而剪影是这一套唯一的表达 |
| 地平线 | `fogDensity: 0.092`（全场最高）。天幕和地面同色时，地平线是唯一会露馅的地方 |
| 体积感 | 不要。侧光一强就有了房间，而这一套的立意是没有房间 |
| "它站在什么上面" | 只剩接触阴影，所以 `contact` 提到 0.72（全场最高） |
