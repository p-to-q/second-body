...
另外量到一处**不是跳变**、但读起来像顿一下的地方：景别是线性进度套 smoothstep，走到一半反向（退后中 → 超时回上半身）时速度一帧从 +1.5/s 翻到 −1.5/s（探针：**一帧 3.0/s**）。位置是连续的，本轮不改，登记在 6.8。

### 6.3 裁定的行为

**一、连续性（S1、S2）。** 任何一个取景状态的变化 —— 景别、小屏放大缩小、跟随开关、腿的站姿混入与放开、控件条上换策略 —— 都不许一帧跳完。
全部由纯函数按时间推（线性进度 + smoothstep、闭式临界阻尼弹簧、限速），不依赖任何 CSS 过渡。守卫：任意决策序列下，
...
- `hold`（降级 / 调速器 / 无人降帧）只冻结跟随，**景别照常用 1 秒走完**。一段按时间推的 1 秒运镜在低帧率下是几步大一点的台阶，不是卡住；一帧切才是真的跳。
...
- **没有做的事**：没有拿真摄像头测过实际能移动多少米、跟随的延迟有多少帧。§6.8 第 2 条、
  §6.3 二里 "横向折算用一个躯干长 = 0.5 米" 那条（§6.8 第 3 条）都还是没有真人量过的数。
