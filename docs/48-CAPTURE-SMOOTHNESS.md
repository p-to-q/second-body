...
|---|---|---|---|---|
| 权限给了（getUserMedia 返回） | 564 | 1580 | 〔〕 | 〔〕 |
| 小屏幕上出现画面 | 1217 | 15553 | 〔〕 | 〔〕 |
| 第一次认出人 | 1254 | 15818 | 〔〕 | 〔〕 |
| 按下之后的长任务（>50ms） | 8 个，共 1250ms | 6 个，共 1181ms | 〔〕 | 〔〕 |
| 最长的一次冻结 | 303ms 建图 · 267ms 首次 detect | 599ms 建图 · 266ms 首次 detect | 〔〕 | 〔〕 |
...
| 造物 · 忒修斯替换 | 替换那一帧 | 基线 +2.05s 一次 102ms（`safe-frame` 81ms） | 调速器第 2 级延后（最多 4 秒，不取消） |
| 造物 · 升档 remorph | 升档那一帧 | 未单独量 | **没做**：见 §8 |
...
1. **管线预编译**：进舞台头几帧有 115–136ms 的帧循环长任务（§4.1）。`renderer.compileAsync(scene, camera)`
   可以在加载态里、或选择页聚焦某个物种时先编译。要先量清楚那几帧里是管线编译还是 remorph。
