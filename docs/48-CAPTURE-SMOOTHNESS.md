...
2. **换件 / 升档那一帧**：采集挪走之后，改后剩下的长任务全部在帧循环里、集中在弧线 30–47 秒（75–153ms，§2.2）。
...
3. **调速器拨的开关本身的代价**：`[governor] @秒` 行对上 `long-animation-frame` 的时刻，确认关后期 / 改像素比那一帧是不是长任务；
...
1. **关后期不拆链**（`stage.setPost`）：原来是 `post.dispose()`，拿回后期那一帧整条链重建。现在只是 `render()` 不走它。
...
我试过在 main 的原样构建上重跑一场做对照（M 场），那一份构建在无头 Chrome 里两次都卡在舞台出现之后、截不到一帧，
没有追下去 —— 所以这张表不宣称"只差这一轮的改动"。
...
| `test/warm-wire.test.ts` | `setPost(false)` 不 dispose；`warmDirect` 用 `compileAsync` 编舞台的 scene / camera；帧循环按 warm-plan 调它并把结果交回 |
