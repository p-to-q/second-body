...
3. ~~**调速器拨的开关本身的代价**~~ **已量已修（§10）**：关后期那一帧 273ms + 367ms，是直出管线第一次被用；改像素比那一帧不是长任务。
   现在关后期不拆链、直出在空闲里预编译。
...
静止 / 大动作两段都是"摄像头在跑、弧线在走"，可比。** 我曾在 main 的原样构建上重跑对照（M 场），走的是选择页那条路、落在 `autonomous`，
两次都在舞台出现之后截不到一帧 —— 那一条单独记在 §10.6，没有追到根。
