...
| **楷书子集覆盖面 `scripts/font-coverage.mjs`** | `stable` | `NOTICE.md` 原来写「子集里 1,321 个汉字」，实测 **958** —— 子集化只做过一次，文案一直在长，叙事中文于是半楷半黑。现在字表是算出来的：扫 `packages/app` 下的 CSS 找出所有用 `var(--sb-kai)` 的选择器（六条），按登记表取对应文案的汉字，与子集 cmap 对账；多一条没登记的选择器就报警。重新子集化后 **319,956 → 211,148 字节 / 958 → 769 个码位 / 770 个字形**，字表带整本 `i18n.ts` 的余量。取证是**真浏览器逐字比像素**（Chrome 152，`document.fonts` 里 Iansui = `loaded`；同一个字用 `64px Iansui, <grotesk>` 与 `64px <grotesk>` 画进 canvas，位图相同 = 没走楷书 —— 汉字两边都是 1em，比字宽量不出来；无头 Chrome 根本不加载这个 webfont，截图会骗人）：首屏立意句 15/15 走楷书（改前 12/15），`/about` 78 个字里 61 个走楷书。**剩下 17 个仍然落回 grotesk，且子集化救不了** —— 「关 观 对 实 验 选 过 们 …」这类简体专用字上游 v1.020 压根没有（芫荽改自日文 Klee One，补的是繁体）。根治要换字（霞鹜文楷，同为 OFL），是一次选型决定，没有顺手做 |
