...
## 一个物种的身份被拆在三处，而其中两处不看参考图

2026-09-13 加三个受邀角色时实测出来的，写下来省得下一个人再走一遍。

写 roster 条目时，`look` 那段英文读起来像"这个物种长什么样"。
**它不是。** 它只被喂给生成模型，而生成模型只产出**几何**。

| 身份的组成 | 来自哪里 | 参考图管不管 |
|---|---|---|
| **形状 / 体积 / 分缝** | 参考图 + `look` 里的形状词 | ✅ 管 |
| **颜色 / 粗糙度 / 金属度** | roster 的 `palette` → `genome` 按 tier 抽 3 个 → `core/palette.ts` 向主色收敛 | ❌ **完全不管** |
| **描边 / 卡通着色 / 轮廓线** | **没有落点** —— 渲染器不支持 | ❌ 不管，而且做不到 |

三条实证：

1. **`char.tokusatsu`**：参考图是镀铬甲片 + 红蓝分色，`look` 里写满了
   "mirror-polished chrome / cobalt-blue"。出来是**金褐色**。
   因为 `docs/03 §4` 明确规定流水线**去贴图、统一套自己的材质** ——
   参考图的颜色从来没有机会到达输出。
2. **`char.line`**：参考图的全部辨识度是**每条轮廓边上一圈粗黑线**。
   出来是一个圆润的白胖子，一条线都没有。
   **描边是着色属性，不是几何属性** —— 它活在着色器里，网格载不动它。
   这个物种不能靠更好的素材解决，它需要渲染器加一层描边（反向外壳是最便宜的做法）。
3. **`char.painting`**：参考图给整张画 → 躯干上长出一张脸；只给衣褶 → 躯干变成平板。
   两张一起给才成（`docs/30 §5.5`）。

### 所以写条目的时候

- `look` 里**只写形状**：体积怎么鼓、分缝在哪、比例如何、边是硬是软。
  颜色词写了不报错，但一个字都不起作用
- 颜色要在 `palette` 里挑**已经存在的材质**（看 `parts.json` 的 `materials`），
  而且注意 **`palette` 是偏好不是保证** —— `genome` 从**当前 tier 可用**的材质里抽，
  tier 1 时 tier 2 的材质根本不在候选里
- 需要新的着色语言（描边、次表面、各向异性）**先改渲染器**，不要指望参考图
