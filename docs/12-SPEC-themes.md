...
```
理想人形 ──→ 真实工业机 ──→ 不该直立的四足机 ──→ 生物机械 ──→ 不可归类 ──→ 无实体
porcelain    industrial      patrol             xeno         coral       field
```

| id | 名 | 一行 | 承担的张力 | 取材 |
|---|---|---|---|---|
| `porcelain` | 瓷 | 一个被理想化过的你 | 干净、无品牌、无磨损，不安来自太完美 | Optimus Gen2/3：白/珍珠聚合物面板 + 黑关节罩 + 银色点缀 |
| `industrial` | 工业 | 它是来上班的，不是来陪你的 | 没有为讨好人做过任何设计 | Figure 02 哑光黑 + 线缆内走；Digit 极简轻量肢 + 传感器密集躯干；Fourier GR-2 银灰 + 黑关节 |
| `patrol` | 巡逻 | 一个不该直立的东西直立了 | 四足机的语言长在人身上 | Unitree Go2 / Spot / ANYmal：方盒躯干、细管执行器、膝部电机鼓包、工程黄黑、无脸 |
| `xeno` | 异形 | 它在模仿你，但模仿错了 | 恐怖谷不在脸上，在关节上 | 生物机械造型传统 |
| `coral` | 珊瑚 | 既不是人，也不是机器 | 最接近原作的不可归类 | 层叠板片 / 鳍 / 高频细节 |
| `field` | 场 | 身体消失，只剩运动 | 玩过前五个之后，让人意识到一直在动的是自己 | 程序化，不生成 |

> **取材 ≠ 复制。** 我们要的是"看得出出身"，不是"认得出品牌"。
> 具体做法见 §4 和 `assets/refs/README.md`。
