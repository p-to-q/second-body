...
| `?cam=…` | — | **Placeholder — do not rely on this row.** A flag for choosing between a built-in and an external camera is being added in another lane right now. It is not in `readFlags()` as this file is written, and its exact name, values and default are not yet known. When it lands, whoever lands it fills this row in from the code | — |
