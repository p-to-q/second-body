...
import {
  imageToStageX, lateralEvidence, stepLateral, LATERAL_REST, type LateralState,
...
test('贴边但整个人都在画里：不报侧边、照常跟随，走到舞台余量的边上为止', () => {
  ...
  assert.ok(last(s).x.x >= 1.0 - AUTOFRAME.lateralDeadZone - AUTOFRAME.lateralBand, `没走到余量边上：${last(s).x.x}`);
