...
export const NASCENT = {
  /**
   * 总开关。**现在是 `false`，也就是开场仍然是改这版之前那套兜底几何。**
   * 下面整组数、以及 `app/src/creature/nascent.ts` 那一整个模块，都在等它翻成 `true`。
   *
   * 为什么是 false：**团块渲染器现在跑不满帧**，而开场那一秒唯一不能丢的就是
   * "那是我在动"。同一台机器、同一分钟内的对照（`?demo=1&tier=0&debug=1`）：
   *
   * | 开场走什么 | fps | 实例 / 面 / draw |
   * |---|---|---|
   * | 兜底刚体件（`enabled:false`） | **76–100** | 30 / 15k / 11 |
   * | 团块（`enabled:true`）        | **12**     | 0 / 2k / 1 |
   *
   * 面数和 draw 都降了一个量级，帧率反而掉到八分之一 —— 所以这不是画得太多，
   * 是别的地方在停。CPU 采样：主线程 **93% 空闲**，JS 每帧只有 1.2–2.4ms。
   * 瓶颈在 GPU 提交那一侧，不在 JS。
   *
   * **这不是本次改动引入的缺陷，是本次把它暴露出来了：**
   *  - `/dev/mass.html?res=36`（本次一行没动）自报 `frame 2.42ms`，却只有 16 fps；
   *    res 降到下限 16 也只有 32 fps —— `res` 这个降级旋钮救不回来。
   *  - 主程序 `?plan=mass`（coral / xeno / dumpling 走的那条路）同样是 10 fps。
   *
   * 也就是说 B 档团块方案**从来没有真正跑到过帧率**，只是没人发现：它的 HUD 报的是
   * 毫秒，而毫秒一直很好看，只有 fps 那一行会说实话 —— 而 `docs/18 §7` 的取证配方
   * 拍的是静帧，静帧里看不出这件事。
   *
   * 翻开它的前置条件只有一条：**修掉 `mass.ts` 每帧更新几何时的那次 GPU 停顿**，
   * 让 `/dev/mass.html?res=36` 稳定在 55 fps 以上。那件事三个团块物种同样受益，
   * 而且和下面这组形态参数完全无关 —— 形态那部分已经做完并取过证，
   * 见 `scratch/evidence/tier0-*.png`。
   *
   * 为什么不先上着等修：12 fps 的身体不比方块躯干好。方块说的是"它坏了"，
   * 卡顿说的也是"它坏了"，而且更响 —— 而这一版要换掉的正是那句话。
   */
  enabled: false,
  /**
