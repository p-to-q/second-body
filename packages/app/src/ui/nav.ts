...
export interface NavOptions {
  ...
  overlay?: boolean;
  /**
   * 挂上来就是展开的。**只有作品那一页该传 true，而且只在展签真的让位时。**
   *
   * 展签的排版在够宽时为它让出了右边一栏（`shell/entry.css` 的 `html.sb-has-nav`
   * 那条），所以在那一页上它铺开来不挡任何东西，反而第一眼就把六个面摆出来了。
   * 但那一栏本身是有断点的（`entry.css` 的 `@media (max-width: 620px)`）——
   * 手机和不少旧笔记本的视口比它窄，那时候展签不再让位，调用方（`main.ts`）
   * 因此要在这个值里叠上一次同断点的 `matchMedia` 判断，不能只看"是不是展签页"。
   * 这一层不重复那个判断，只按传进来的值行事。
   *
...
export function mountNav(options: NavOptions = {}): Nav | null {
  ...
  // 「目录」这个词在每一张挂目录的页上是同一件东西：换页时它不动（docs/47）
  declareShared(toggle, 'nav');
