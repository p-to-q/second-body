...
export const SCENES: Record<SceneId, SceneDef> = {
  ...
  paper: {
    ...
    // 上下同色 = 无缝。不是"天空和地面恰好接近"，是真的同一张纸。
    //
    // 给 1.32 而不是 #fafafa 的线性值 0.9559：色调映射会把线性值往下压，
    // 直接给 0.9559 出来是浅灰。要让**编码之后**落在 #fafafa 上，
    // 进去的那个数就得高一些。这个数是看出来的，不是算出来的 ——
    // 色调映射曲线不是一个可以反解的函数，所以改了要重新看一眼。
    ...
    fogDensity: 0.092,
    // `ground` 对这一套**没有作用**：`applyScene` 看到 tint 为 0 就直接用天幕，
    // 一个乘法都不做。留着是因为 SceneDef 要求有这个字段 —— 别去调它。
...
export function applyScene(look: LookProfile, scene: SceneDef): LookProfile {
  ...
  return {
    ...
    //
    // `tint: 0` 的那一套（纸）两头都不对：
    //  · 乘物种的地色 → 上半屏中性、下半屏带色，雾一混整块底就暖了
    //    （实测 char.line 站上去，那张"纸"是米黄的）；
    //  · 乘纸自己的颜色 → 那几个倍数（5~6）是按暗色地色配的，
    //    乘在已经接近 1 的值上直接爆掉，连身体带影子一起冲没。
    // 所以它**直接等于天幕**，一个乘法都不做 —— 那才是"无缝的一张纸"的
    // 字面意思，也省掉一个需要人去配平的数。
    //
    // 于是 `tint` 管的不只是天幕，是**这一套底色认不认得站上来的是谁**。
