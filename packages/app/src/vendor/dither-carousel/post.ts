...
// Verbatim apart from TypeScript annotations.
import * as THREE from "three";
import {
  fullscreenVertex,
  streakFragment,
  compositeFragment,
} from "./shaders/composite.ts";
import { hexToSRGB } from "./color.ts";
import type { CarouselConfig } from "./config.ts";
...
export interface PostPipeline {
  ...
  readonly targets: { sceneTarget: THREE.WebGLRenderTarget; blurTargets: THREE.WebGLRenderTarget[] };
}
...
export function createPostPipeline(
  ...
  let blurTargets: THREE.WebGLRenderTarget[] = [];
  let scratchTargets: THREE.WebGLRenderTarget[] = [];
