...
// Changes from upstream gl/config.js:
//   - typed for TS; `bendMode` is a union rather than a bare string
//   - the IMAGES / PROJECTS placeholder arrays are gone. They pointed at the
//     upstream public/ artwork, which is NOT under the MIT licence (see the
//     note at the bottom of ./LICENSE). Cards come from the caller instead.
//   - the lil-gui panel that used to write these live is not ported, so these
//     are edit-and-reload constants now.
...
export const config = {
  ...
  // starting sooner spends the arrival on empty rectangles. Call `replayEntry()`
  // on the handle to see it again without reloading.
  entry: true,
  ...
  bend: 2.7,
  bendMode: 'horizontal' as BendMode,
  bendEase: 0.12,
...
export type CarouselConfig = typeof config;
