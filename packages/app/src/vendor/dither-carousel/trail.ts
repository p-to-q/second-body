...
import { fullscreenVertex } from "./shaders/composite.ts";
import { trailFragment } from "./shaders/trail.ts";
...
export function createTrail(
  ...
  function setSize(w: number, h: number, pixelRatio: number) {
    width = Math.max(1, w);
    height = Math.max(1, h);
