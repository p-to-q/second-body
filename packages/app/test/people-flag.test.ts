...
test('?people= 只认 1..hardMax 的整数；别的当没写过，走场合默认', () => {
  ...
  assert.equal(readFlags('?people=9').people, PEOPLE.defaultCap, '认不出来 = 默认，不是夹到上限');
...
test('控件「人数」：每个选项写进 URL 再用 readFlags 读回来是同一个值；默认值写成删除', () => {
  ...
  assert.equal(c.default, String(readFlags('').people), '表的默认 = 不带参数开机读到的');
  ...
  for (const v of c.options as readonly string[]) {
    ...
    assert.equal(String(readFlags(`?${q}`).people), v, `people=${v} 写出去读不回来：${q}`);
    if (Number(v) === PEOPLE.defaultCap) assert.equal(patch.people, null, '默认值不留在地址栏里');
  }
