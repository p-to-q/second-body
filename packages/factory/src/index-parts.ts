...
import { THEMES } from '../recipes/themes.ts';
...
export async function buildIndex(): Promise<PartLibraryIndex> {
  ...
  const index: PartLibraryIndex = {
    ...
    themes: THEMES.map((t) => ({ id: t.id, name: t.name, nameEn: t.nameEn, tagline: t.tagline, palette: t.palette, source: t.source })),
    materials: MATERIALS,
  ...
  console.log(`parts.json: ${parts.length} 件, ${MATERIALS.length} 材质, ${THEMES.length} 主题`);
