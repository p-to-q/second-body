 ...
 * 只降分辨率 + 调色板量化 —— 对一个**抖动**轮播（dither-carousel）来说这两件事几乎看不出来。
 *
 * 为什么是 768 / 128 色：卡片是 1024×512 的 cover 裁切（`cards.ts` CARD_W/CARD_H），
 * 源图只有横向 1024 会被用满；768 上采样 1.33× 落在抖动噪声以下。实测 21 张总量 6.12 MB → 1.00 MB。
 *
